# Cover Letter Feature - Supabase Schema

This file contains all SQL needed to set up the cover letter feature in Supabase.
...
## 3. Insert 30 Pre-written Templates
...
-- 30. UX/UI Designer
('UX/UI Designer', 'Technology', 'Mid-level',
'[Your Address]
[Phone Number]
[Email Address]
[Date]
...
Creating intuitive, user-centered experiences that solve real problems is what drives my passion for UX/UI design. With [Years of Experience] years of experience designing digital products, I am excited about the UX/UI Designer opportunity at [Company Name].

At [Previous Company Name], I redesigned a core product feature that increased user engagement by 64% and reduced customer support tickets by 42%. I conducted user research with over 150 participants that informed design decisions, leading to a mobile app redesign that improved task completion rates by 78%. My design system implementation created consistency across 12 products while reducing design and development time by 35%.

What draws me to [Company Name] is your commitment to [Specific Company Achievement/Project]. Your product demonstrates thoughtful design and attention to user needs that I admire. I am eager to contribute my user research expertise and design skills to create experiences that delight your users.

With [Relevant Certification] and proficiency in Figma, Sketch, Adobe XD, user testing tools, and front-end HTML/CSS, I bring both design thinking and technical understanding. I excel at conducting user research, creating wireframes and prototypes, collaborating with product and engineering teams, and advocating for users throughout the product development process. My approach combines data-driven insights with creative problem-solving.

I would welcome the opportunity to discuss how my UX/UI expertise can enhance [Company Name]''s product experience. Thank you for your consideration.

Sincerely,
[Full Name]',
'Creating intuitive, user-centered experiences that solve real problems is what drives my passion for UX/UI design. With years of experience designing digital products...');
```
...
```sql
-- Check that all 30 templates were inserted
SELECT COUNT(*) as total_templates FROM cover_letter_templates;

-- View all templates grouped by industry
SELECT industry, COUNT(*) as count
FROM cover_letter_templates
GROUP BY industry
ORDER BY count DESC;

-- View all templates grouped by experience level
...
SELECT job_title, industry, experience_level,
       LEFT(preview_text, 100) as preview
FROM cover_letter_templates
ORDER BY job_title
LIMIT 10;
```
...
```sql
-- Test as an authenticated user (run these after logging in via Supabase Auth)
-- This should return all templates (public read access)
SELECT COUNT(*) FROM cover_letter_templates;

-- This should return 0 (no saved letters yet)
SELECT COUNT(*) FROM user_cover_letters WHERE user_id = auth.uid();

-- Insert a test cover letter (should succeed)
INSERT INTO user_cover_letters (user_id, template_id, title, customized_content)
SELECT
  auth.uid(),
  id,
  'Test Cover Letter',
...
SELECT * FROM user_cover_letters WHERE user_id = auth.uid();
```
...
After running all SQL commands above, you will have:

1. ✅ **2 tables created**: `cover_letter_templates` and `user_cover_letters`
2. ✅ **30 pre-written templates** inserted covering all major job roles
3. ✅ **RLS policies** protecting user data
4. ✅ **Indexes** for optimized queries
5. ✅ **Auto-update timestamps** via triggers

**Next Steps:**
- Integrate with React frontend using Supabase client
- Create UI components for browsing, editing, and saving cover letters
